...
Movie synchronization stores Radarr movie IDs, TMDb IDs, movie-file IDs, title metadata, a Radarr web path based on `tmdbId` when available, monitored state, file presence, source paths, relative paths, quality, quality profile, file size, status and poster metadata where available.

Radarr browser links are generated as `/movie/<tmdbId>` when the API returns `tmdbId`, matching Radarr deployments that expose movie details such as `/movie/1273002`. Czecharr does not fall back to the internal Radarr movie ID because that can open the wrong Radarr route.
